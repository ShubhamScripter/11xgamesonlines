...
function AffiliateDashboard() {
  ...
  const [report, setReport] = useState({ byUser: [], totalBalance: 0 });
  const [weekly, setWeekly] = useState({ weeklyCommission: 0 });
  ...
  const loadAgent = async () => {
    const [dl, rep, wk, ref] = await Promise.all([
      axios.get('/agent/affiliate/downline'),
      axios.get('/agent/affiliate/commission-report'),
      axios.get('/agent/affiliate/weekly-commission'),
      axios.get('/agent/affiliate/referral-link'),
    ]);
    setDownline(dl?.data?.data || []);
    setReport(rep?.data?.data || { byUser: [], totalBalance: 0 });
    setWeekly(wk?.data?.data || {});
    setReferralLink(ref?.data?.data?.referralLink || '');
  };
  ...
  if (!isAdmin && !isAgent) {
    return (
      <div className="p-4 text-[#243a48] font-['Times_New_Roman']">
        Affiliate dashboard is for admins and agents only.
      </div>
    );
  }
  ...
  return (
    <div className="mt-4 p-2 font-['Times_New_Roman'] max-w-6xl">
      <h1 className="text-[#243a48] text-[16px] font-[700]">Affiliate / Agent System</h1>
      <p className="text-sm text-gray-600 mt-1">
        Agents earn commission on referred user losses. Commission is credited automatically on bet settlement.
      </p>
        ...
        <div className="space-y-4 mt-4">
          <section className="bg-[#e0e6e6] border-b border-b-[#7e97a7] p-4">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Status: {settings.enabled ? `Active — ${settings.globalCommissionPercent || globalPct}% default` : 'Disabled'}
            </p>
          </section>
          ...
          <section className="bg-white border border-[#7e97a7] overflow-x-auto">
            <h2 className="text-[#243a48] font-[700] p-3 border-b border-[#7e97a7]">Agents ({agents.length})</h2>
      ...
      {isAgent && (
        <div className="space-y-4 mt-4">
          <section className="bg-[#e0e6e6] border-b border-b-[#7e97a7] p-4">
            <h2 className="text-[#243a48] font-[700] mb-2">Your Referral Link</h2>
            <div className="flex flex-wrap items-center gap-2">
              <p className="text-sm break-all font-mono bg-white border border-[#aaa] p-2 rounded flex-1 min-w-0">
                {referralLink}
              </p>
              <button
                type="button"
                onClick={() => copyText(referralLink)}
                className="bg-[#243a48] text-white px-4 py-2 rounded text-sm font-semibold"
              >
                Copy Link
              </button>
            </div>
            <p className="text-xs text-gray-600 mt-2">
              Share this link with new users. They will be attached to your downline on registration.
            </p>
          </section>

          <section className="grid md:grid-cols-2 gap-4">
            <div className="bg-[#e0e6e6] border-b border-b-[#7e97a7] p-4">
              <p className="text-gray-600 text-sm">Total Commission Earned</p>
              <p className="text-2xl font-bold text-[#243a48]">{report.totalBalance}</p>
            </div>
            <div className="bg-[#e0e6e6] border-b border-b-[#7e97a7] p-4">
              <p className="text-gray-600 text-sm">This Week&apos;s Commission</p>
              <p className="text-2xl font-bold text-[#243a48]">{weekly.weeklyCommission ?? 0}</p>
            </div>
          </section>

          <section className="bg-white border border-[#7e97a7] overflow-x-auto">
            <h2 className="text-[#243a48] font-[700] p-3 border-b border-[#7e97a7]">
              Downline Users ({downline.length})
            </h2>
            <table className="w-full text-sm">
              <thead className="bg-[#e0e6e6]">
                <tr>
                  <th className="text-left p-2">User</th>
                  <th className="text-left p-2">Balance</th>
                  <th className="text-left p-2">Av Balance</th>
                  <th className="text-left p-2">Betting P/L</th>
                </tr>
              </thead>
              <tbody>
                {downline.map((u) => (
                  <tr key={u._id} className="border-t border-[#e0e6e6]">
                    <td className="p-2">{u.userName}</td>
                    <td className="p-2">{u.balance}</td>
                    <td className="p-2">{u.avbalance}</td>
                    <td className="p-2">{u.bettingProfitLoss}</td>
                  </tr>
                ))}
                {!downline.length && (
                  <tr>
          ...
          <section className="bg-white border border-[#7e97a7] overflow-x-auto">
            <h2 className="text-[#243a48] font-[700] p-3 border-b border-[#7e97a7]">Commission by User</h2>
            <table className="w-full text-sm">
              <thead className="bg-[#e0e6e6]">
                <tr>
                  <th className="text-left p-2">User</th>
                  <th className="text-left p-2">User Loss</th>
                  <th className="text-left p-2">Your Commission</th>
                  <th className="text-left p-2">Events</th>
                </tr>
              </thead>
              <tbody>
                {(report.byUser || []).map((row) => (
                  <tr key={row.userId} className="border-t border-[#e0e6e6]">
                    <td className="p-2">{row.userName}</td>
                    <td className="p-2">{row.totalUserLoss}</td>
                    <td className="p-2 font-semibold text-green-700">{row.totalCommission}</td>
                    <td className="p-2">{row.eventCount}</td>
                  </tr>
                ))}
