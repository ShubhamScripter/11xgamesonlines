...
function Casino() {
  const navigate = useNavigate();
    const casinoProviders = Object.entries(casinoData.providers)
    .filter(([_, games]) =>
      games.some((g) =>
        g.game_type?.toLowerCase().includes("casino")
      )
    )
    .map(([key]) => key);

  const providers = ["all", ...casinoProviders];
    ...
    <div className="bg-[#141515] px-4">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-2 pb-4">
        {providers.map((p) => (
          <div
            key={p}
            onClick={() => navigate(`/casino/casino/${p}`)}
