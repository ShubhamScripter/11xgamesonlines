...
import jiliGames from '../../../components/api_json/Evolution.json'
import { startCasinoGame, getCasinoWalletAmount } from '../../../services/casinoService';import { getFishingImage } from '../../../components/FishingPics'
import Spinner from '../../Spinner'
function Evolution() {
  const { user } = useSelector((state) => state.auth);
  const [loading, setLoading] = useState(false);
  
  // Filter Fish Games and get remaining games for AtoZ (skip first 10)
  const fishGames = jiliGames.filter(game => game.game_type === 'CasinoLive');
  ...
  return (
    <div className='grid grid-cols-3 gap-2'>
        {fishGames.map((game, index) => (
          ...
          >
            <img 
            //   src={getFishingImage(game.iconlink)} 
            src={game.icon}
              alt={game.game_name} 
