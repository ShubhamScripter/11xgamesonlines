...
import jiliGames from '../../../components/api_json/BigTimeGaming.json'
...
function BigTimeGaming() {
  const { user } = useSelector((state) => state.auth);
  const [loading, setLoading] = useState(false);
  
  // Filter Slot Games and get remaining games for AtoZ (skip first 16)
  const slotGames = jiliGames.filter(game => game.game_type === 'Slot')
