...
import { startCasinoGame, getCasinoWalletAmount } from '../../services/casinoService';import { getSlotImage } from '../SlotPics'
import Spinner from '../Spinner'
function Latest() {
  const { user } = useSelector((state) => state.auth);
  const [loading, setLoading] = useState(false);
  
  // Filter Slot Games and get first 16 for Latest
  const slotGames = jiliGames.filter(game => game.game_type === 'Slot Game').slice(0, 16);
