...
function SoccerOver15({ openBetSlip, matchOddsList, gameid, match, selectedBetData, gameName }) {
  ...
  const getBetDetails = (pendingBet, team) => {
    ...
    const matchedTeamBet = pendingBet?.find(
      (item) => item.teamName?.toLowerCase() === team?.toLowerCase() && item.gameType === "OVER_UNDER_15"
    );
  ...
  return (
    <div className="bg-white">
      <div className="bg-[#17934e] h-10 p-2 pl-4 flex justify-between items-center">
        <div className="flex items-center gap-2">
        <GrStarOutline className="text-white" />
        <span className="text-white">Over/Under 1.5</span>
        </div>
