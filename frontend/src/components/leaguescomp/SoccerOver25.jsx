...
function SoccerOver25({ openBetSlip, matchOddsList, gameid, match, selectedBetData, gameName }) {
  ...
  const getBetDetails = (pendingBet, team) => {
    ...
    const matchedTeamBet = pendingBet?.find(
      (item) => item.teamName?.toLowerCase() === team?.toLowerCase() && item.gameType === "OVER_UNDER_25"
    );
